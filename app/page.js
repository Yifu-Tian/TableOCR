...
export default function Home() {
  ...
  const handleChange = async (e) => {
    ...
    if (file && file.type === 'application/pdf') {
      setIsRendering(true);
      try {
        const imageSrc = await renderPdfPageWithTable(file, setProgress); // 调用渲染PDF图像的函数
    ...
    <main className="container py-8 px-6 flex flex-col min-h-screen items-center justify-center gap-4">
      <div className="flex flex-col w-full max-w-lg items-end gap-4">
        <div className="w-full p-6 shadow-md rounded-md border border-black/30">
          ...
          <form id="test-form" onSubmit={handleSubmit} className="w-full flex flex-col gap-4">
            <input type="file" name="file" accept="application/pdf,image/png,image/jpg,image/jpeg" formEncType="multipart/form-data" required onChange={handleChange}/>
      ...
      {imageSrc && (
        <div>
          ...
          <img src={imageSrc} alt="PDF Page Preview" className="w-full h-auto" />
        </div>
      )}
      <ListData/>
  </main>
